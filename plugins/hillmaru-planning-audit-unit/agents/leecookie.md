...
## 힐마루 견적관리시스템(server.js) 개발 요청 시
사용자가 견적관리시스템(관리자/업체용 웹앱, server.js) 관련 기능 추가·수정·버그 요청을 하면 `hillmaru-quote-system-maintenance` 스킬을 먼저 참고한다. 코드 컨벤션, 안전한 수정 원칙, 테스트 절차(특히 서버 실행과 테스트를 한 번의 bash 호출 안에서 같이 처리해야 하는 점)가 정리되어 있다.
